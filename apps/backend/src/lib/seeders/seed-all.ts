...
/**
 * seed-all.ts — ORQUESTACIÓN DE SEEDERS
 * ---------------------------------------------------------------------------
 * Contiene el arreglo STEPS y la función `seedAll()`. Vive en un archivo
 * SEPARADO de `index.ts` a propósito:
 *
 * Este archivo (y todo lo que importa, incluyendo cada *.seeder.ts y, a
 * través de ellos, `@reto/db`) SOLO debe cargarse DESPUÉS de que `index.ts`
 * haya llamado a `dotenv.config()`. `index.ts` lo importa con
 * `await import('./seed-all.js')` (import DINÁMICO) en vez de un import
 * estático, precisamente para garantizar ese orden — ver el comentario en
 * index.ts para el detalle de por qué un import estático rompe esto.
 * ---------------------------------------------------------------------------
 */
