...
async function notifyTechnicianAssigned(caseId: number, technicianId: number, isReassignment: boolean, notes?: string) {
  const title = isReassignment ? `Caso reasignado: #${caseId}` : `Nuevo caso asignado: #${caseId}`;
  const actionText = isReassignment ? 'ha sido reasignado' : 'ha sido asignado';

  let establishmentName = 'Establecimiento Regulado';
  let priority = 'MEDIA';
  try {
    ...
    // Si falla, se usan valores por defecto
  }

  const message = `Se te ${actionText} el caso #${caseId} (${establishmentName}). Prioridad: ${priority}.${notes ? ` Notas: ${notes}` : ''} Revisa los detalles en el sistema para programar la evaluación sanitaria.`;
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1.5px solid #00236f; border-radius: 8px; overflow: hidden; background: #ffffff;">
      <div style="background: #00236f; color: #ffffff; padding: 18px 24px;">
        ...
        <p style="margin: 4px 0 0; color: #93c5fd; font-size: 12.5px;">Sistema de Vigilancia Sanitaria y Evaluación Basada en Riesgo</p>
      </div>

      <div style="padding: 24px; color: #1e293b;">
        <h3 style="color: #00236f; margin-top: 0; font-size: 16px;">
          ${isReassignment ? '🔄 Caso Reasignado a tu Cargo' : '📋 Nuevo Caso Asignado para Inspección'}
        </h3>
        <p style="font-size: 14px; line-height: 1.6; color: #334155;">
          Estimado/a Técnico Evaluador, se le ${actionText} formalmente el siguiente expediente para su atención e inspección técnica:
        </p>
        ...
        <div style="background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 6px; padding: 16px; margin: 18px 0; font-size: 13.5px; line-height: 1.6;">
          <div><strong>Expediente / Caso:</strong> #${caseId}</div>
          <div><strong>Establecimiento:</strong> ${establishmentName}</div>
          ...
          ${notes ? `<div style="margin-top: 6px; color: #475569;"><strong>Instrucciones / Notas:</strong> ${notes}</div>` : ''}
        </div>

        <p style="font-size: 13px; color: #64748b; line-height: 1.5;">
          Ingresa al panel operacional para coordinar la inspección en campo, registrar la ficha BPM y capturar las evidencias.
        </p>
      </div>
      ...
      <div style="background: #f1f5f9; padding: 12px 24px; font-size: 11.5px; color: #64748b; text-align: center; border-top: 1px solid #e2e8f0;">
        República Dominicana • Ministerio de Salud Pública y Asistencia Social (MISPAS)
      </div>
    </div>
  `;
