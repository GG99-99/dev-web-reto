...
const navForRole = (role: Role) => nav.filter((item) => {
  ...
  if (item.id === 'company') return role === 'ADMIN_EMPRESA' || role === 'USUARIO_DELEGADO' || role === 'ADMIN' || role === 'COORDINADOR'
  return true
})
...
function App() {
  ...
  const [view, setView] = useState<View>(() => {
    try {
      ...
      if (parsed?.role === 'ADMIN_EMPRESA' || parsed?.role === 'USUARIO_DELEGADO') return 'company'
      return 'overview'
    } catch {
      return 'overview'
    }
